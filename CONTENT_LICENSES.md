# Content Licenses and Attribution

JLPT Coach Version 2.1.1 does not bundle or reproduce complete official JLPT past examinations.
The expanded Vocabulary/Kanji pack is built at runtime from the following open sources.

## 1. OpenJLPT

- Project: OpenJLPT
- Homepage: https://github.com/evanclan/OpenJLPT
- License: Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)
- Used fields: vocabulary, readings, meanings, examples, kanji readings and meanings
- Required action: retain attribution and share adaptations under compatible terms where CC BY-SA applies

## 2. WordMaster Word Lists

- Project: WordMaster Word Lists
- Homepage: https://github.com/lratusa/wordmaster-wordlists
- License: MIT
- Used fields: JLPT vocabulary and kanji datasets
- Note: some translations are Chinese; JLPT Coach restricts non-English entries to reading/audio-oriented practice where appropriate

## 3. Open Anki JLPT Decks

- Project: Open Anki JLPT Decks
- Homepage: https://github.com/jamsinclair/open-anki-jlpt-decks
- License: MIT
- Used fields: expression, reading, meaning and tags from the N5–N1 CSV files

## 4. JLPT Coach curated starter content

- Project-specific educational content supplied with JLPT Coach
- Used to preserve the original Thai explanations, lesson flow and stable SRS IDs

## Official JLPT materials

Official JLPT websites are consulted only to understand the published test structure, question types and copyright limitations. Complete official test questions, listening audio and copyrighted passages are not copied into this application.

This file is an attribution summary, not legal advice. Repository maintainers should preserve upstream LICENSE/NOTICE requirements when distributing modified data or derived packs.
